...
(function (global) {
    ...
    if (!utils) {
        console.error("[SearchFusion-Iy] Utils not found!");
        return;
    }

    global.SearchFusionEngines.iyf = {
        parse: function () {
            const results = [];
            const engineName = "爱壹帆";
            ...
            nodes.forEach((node) => {
                try {
                    ...
                    console.error(`[SearchFusion-Iy] Error parsing an item:`, e, node);
                }
            });
