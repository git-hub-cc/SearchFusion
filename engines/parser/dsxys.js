/**
 * engines/parser/dsxys.js
 * 大师兄影视 (dsxys.com) 专用解析器
 */
(function (global) {
    ...
    if (!utils) {
        console.error("[SearchFusion-Dsxys] Utils not found!");
        return;
    }

    global.SearchFusionEngines.dsxys = {
        parse: function () {
            const results = [];
            const engineName = "大师兄";
            ...
            nodes.forEach((node) => {
                try {
                    ...
                    console.error(`[SearchFusion-Dsxys] Error parsing an item:`, e, node);
                }
            });
